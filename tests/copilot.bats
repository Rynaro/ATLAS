...
@test "T14: copilot host with no .agent.md files → info log + exit 0" {
  ...
  # No .agent.md files were created. The directory itself must remain
  # absent (the script must not have created .github/agents/ as a side
  # effect), and even if it exists, no *.agent.md files should be in it.
  if [ -d .github/agents ]; then
    local found
    found="$(find .github/agents -name '*.agent.md' 2>/dev/null || true)"
    [ -z "$found" ] || {
