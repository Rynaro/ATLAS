...
ATLAS now also validates emitted envelope sidecars against `schemas/ecl-envelope.v2.json` (vendored from `eidolons-ecl/schemas/envelope.v2.json`). Envelopes carry an `ise` block (`assertion_grade: "self-attested"`).
...
ATLAS targets ECL v2.0 (`ECL_VERSION` file); ECL adoption is additive and opt-in.
...
finds the installed agent at `.eidolons/atlas/agent.md`.
